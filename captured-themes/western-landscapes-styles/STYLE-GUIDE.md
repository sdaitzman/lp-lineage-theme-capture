...
**Source URL (production):** https://landscapepartnership.org/networks/working-lands-for-wildlife/landscapes-wildlife/landscapes/western-landscapes
**Sub-site CSS:** `wlfw-western.css` (16 KB)
**Shared CSS:** `ploneCustom.css` (~130 KB)
**Slug:** `western-landscapes`
**Date last captured:** 2026-05-05

---

## Page Inventory

Numbering matches `html/NN-slug.html` and `screenshots/NN-slug.png` / `NN-slug-mobile.png`.

| # | Page | URL Path (relative to sub-site root) | Body Classes |
|---|------|--------------------------------------|--------------|
| 1 | Home | `/` (western-landscapes) | `site-western-landscapes section-western-landscapes` |
...
### Navigation Structure (Primary, top-level)
- Home → `/`
- About → `/about` (sub: Partners → `/about/partners`)
- Wildlife → `/wildlife`
  - External wildlife species pages (not captured — separate hosts, see Capture notes):
    Sage Grouse [wlfw.org], Lesser Prairie-Chicken [wlfw.org], Southwestern Willow Flycatcher [wlfw.org], Hawaiian Nene [nrcs.usda.gov], Kenai Peninsula Salmon [nrcs.usda.gov]
- Resources → `/resources` (sub: Great Plains Grassland Biome Framework, Sagebrush Biome Framework)
- Training → `/training`
- Workspace → `/workspace`

### Return-to / parent chrome
- Top "Return to Working Lands for Wildlife site" bar (`.wlfw-biome-nav-bar`) — links back to parent WLFW landscapes index.
- "Navigate WLFW Landscapes" biome-switcher menu (`.landscape-biome-menu`) — links to sibling landscape sub-sites (Grasslands and Savannas, Aquatics, Eastern Deciduous Forests, Western Landscapes). Sibling sites have their own captures.

### Footer link pages (LP main footer, shared site-wide; not part of this sub-site)
The page renders the LP-wide `#portal-footer` containing global links (About Us, Members, Community, Issues, Help, Projects, Maps and Data, News and Events, Resources, Training, Contact Us). These pages live outside the sub-site and are captured separately as part of `lp-parent-site`.

### Capture notes
- Source captured from production (`landscapepartnership.org`); never `dev.landscapepartnership.org`.
- **Unreachable: `/resources/maps` (Apps, Maps, and Data)** — listed in earlier captures and historic nav, but as of 2026-05-05 the page redirects to a login challenge (`/acl_users/credentials_cookie_auth/require_login`). Documented here per plan §1.1 and excluded from html/screenshots inventory. Re-evaluate when LP exposes a public Maps page for this sub-site.
- External Wildlife sub-pages are intentionally not captured — they live on third-party domains (`wlfw.org`, `nrcs.usda.gov`) and are not part of the LP-hosted theme.
...
|---|---|---|
| Open Sans | 400, 400i, 600 | googleapis |
| Source Sans Pro | 400, 700 | googleapis |
| Merriweather | 400, 400i, 700 | googleapis |
| Montserrat | 400, 500, 600 | googleapis |
| Material Icons | (icon set) | googleapis |
| Nunito Sans | 400, 700, 400i | googleapis css2 |
| Public Sans | 300, 400, 700, 300i, 400i | googleapis css2 |

> Note: the live site loads the full set above, but most type on this sub-site uses **Public Sans** (body, nav) and **Merriweather** (all headings, blockquote/pullquote). Other families are inherited from LP-wide defaults and only appear in specific overrides (e.g. `.cta-button` uses Montserrat).

---

## Print Styles

**Source:** `css/print.css` only — no `@media print` blocks were found in `wlfw-western.css` or `ploneCustom.css`. The sub-site relies entirely on the LP-wide print stylesheet.

### What's hidden in print (LP base)
...
- `body { font-family: "Helvetica Neue", Arial, FreeSans, sans-serif; }` — overrides Public Sans
- `h1–h6 { font-family: "Helvetica Neue", Arial, FreeSans, sans-serif; font-weight: bold; line-height: 125%; page-break-inside: avoid; page-break-after: avoid; }`
- `h1, h2 { letter-spacing: -0.05em; }`
- `a { text-decoration: none; border-bottom: 0.1em solid gray; color: black; }` (link URLs not auto-printed)
- `.documentDescription { font-weight: bold; }`

### Layout simplifications
- `#portal-column-content { width: 95%; overflow: hidden; }` (single column)
...
- `#content dt { margin: 1em 0 0.625em; }`, `#content p, #content table { margin-bottom: 0.625em; margin-top: 0.625em; }`, `#content .documentDescription, #content #description { margin: 0.5em 0 1.31em; }`
...
### `@page`
- No `@page` rules present — page margins, page size, and named pages are not declared. Browser defaults are used.

### Implications for theme replication
- The base `_print.scss` already covers the LP-wide hides plus LP-specific extras (return-to bar, biome menu, social, etc.) — sub-site override file should add **no print rules** unless a sub-site-specific decorative element (e.g. `#wlfw-west` background block) needs to be hidden or simplified.
- No sub-site-specific print color/typography overrides exist on the live site, so don't introduce new ones during replication. Keep the SCSS comment placeholder noted in the plan.
